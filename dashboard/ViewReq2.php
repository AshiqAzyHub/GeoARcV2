...
session_start();
    ...
    if(isset($_GET['cid'])){
        ...
        $a = "UPDATE cusreq SET Completed='1' where cusid= $cid";
        mysqli_query($link, $a);
        header("location:ArchDash.php");
    }
    if(isset($_REQUEST['update']))
{
    $query="UPDATE cusreq SET Completed='1' where cusid= $cid";	
	}
                      ...
                      <div class="order">
                        <table class="table" method="post">
                          ...
                          <?php
                           $aid = intval($_GET['aid']);
                            $i=1;
                         $sql="select cusreq.cusid,cusreq.Completed,login.fname,login.Date,cusreq.planfor,cusreq.rooms,cusreq.toilet,cusreq.location,cusreq.contact,cusreq.AddInfo,cusreq.budget,cusreq.CusReqStatus from cusreq INNER JOIN login on cusreq.login_id=login.login_id where cusreq.cusid=$aid"; 
                         $query = mysqli_query($link,$sql);
                          if(mysqli_num_rows($query)>0){
                         
                         while($row=mysqli_fetch_assoc($query)){
                         ?> 
                            <tr><th>User Name</th>                <td><?php echo $row['fname'];?></td> </tr>
                            <tr><th>Date of Request</th>                <td><?php echo $row['Date'];?></td> </tr>

                            <tr><th>Design for</th>                <td><span ><?php if($row['planfor']==1){echo"Home";}else if($row['planfor']==2){echo"banglow";}else if($row['planfor']==3){echo"school";} else if($row['planfor']==4){echo"apartment";} ?></span></td></tr>

                            <tr><th>Rooms</th>                <td><?php echo $row['rooms'];?></td> </tr>

                            <tr><th>Toilet</th>                <td><?php echo $row['toilet'];?></td> </tr>

                            <tr><th>Location</th>                <td><?php echo $row['location'];?></td> </tr>

                            <tr><th>Contact</th>                <td><?php echo $row['contact'];?></td> </tr>

                            <tr><th>Additional Info</th>                <td><?php echo $row['AddInfo'];?></td> 


                            <tr><th>Budget</th>                <td><span ><?php if($row['budget']==1){echo"1-50";}else if($row['budget']==2){echo"1 cr";}else if($row['budget']==3){echo"5 cr";} else if($row['budget']==4){echo"10+ cr";} ?></span></td></tr>

                            <tr><th>Status</th>                <td><span ><?php if($row['CusReqStatus']==0){echo"Not Accepted";}else if($row['CusReqStatus']==1){echo"Accepted";} ?></span></td>

                              
                            </tr>
                            <tr><th>Upload Design</th>
                            
                <td>
           <input type="file" name="file" placeholder="Upload Image files only" accept="image/png, image/gif, image/jpeg">
                         </td></tr>
                         <tr><th>Price of Design</th><th><input  type="text" name="price" placeholder="Enter Price Of Design"></th></tr>
                ...
                <td> <a href="ViewReq2.php?cid=<?php echo $row['cusid'];?>" ><li class="onhover-dropdown p-0">
                <button class="btn btn-primary-light" type="button"><i></i>Submit</button> <?php }
                else{
                   ?>
                    <td><li class="onhover-dropdown p-0">
                    <button class="btn btn-primary-light" type="button"><i data-feather="check-circle"></i>Completed</button>
                <?php }?>
        </li>
        </a> </td>
              </tr>
